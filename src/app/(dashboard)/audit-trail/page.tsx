...
type AuditRow = { id: string; source: string; entityType: string; entityId?: string; action: string; message?: string; before?: unknown; diff?: unknown; after?: unknown; ip?: string; userAgent?: string; requestId?: string; createdAt: string; actor?: Actor | null };
const sourceOptions = ['WORKSPACE', 'DOCUMENT', 'ACCESS'];
const moduleOptions = ['ACCESS', 'ACCOUNTING', 'CRM', 'CUSTOMERS', 'HR', 'INVENTORY', 'INVOICING', 'MANUFACTURING', 'PROCUREMENT', 'PROJECTS', 'SALES', 'SUPPLIERS'];
...
function AuditTrailContent() {
  ...
  const [filters, setFilters] = useState({ search: '', source: '', module: '', action: '', userId: '', from: '', to: '', entityId: '' });
  const limit = 25;

  useEffect(() => {
    setPage(1);
    setFilters(current => ({ ...current, module: searchParams.get('module')?.toUpperCase() || '', source: searchParams.get('source')?.toUpperCase() || '' }));
  }, [searchParams]);
  ...
  const columns = [
    ...
    { key: 'source', header: 'Source', render: (row: AuditRow) => <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700">{row.source}</span> },
  ...
  return <div>
    <PageHeader title="Audit Trail" description="One complete, filterable history of workspace, document, and access changes">
    ...
    <div className="mb-4 grid gap-3 rounded-lg border bg-white p-4 md:grid-cols-2 xl:grid-cols-4">
      <div className="space-y-1"><Label>Search</Label><Input placeholder="Record, action, message, or email" value={filters.search} onChange={event => setFilter('search', event.target.value)} /></div>
      <FilterSelect label="Source" value={filters.source} allLabel="All sources" options={sourceOptions} onChange={value => setFilter('source', value)} />
    ...
    <Dialog open={!!selected} onOpenChange={open => !open && setSelected(null)}><DialogContent className="max-h-[85vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>Audit Event Details</DialogTitle></DialogHeader>{selected && <div className="space-y-4 text-sm"><div className="grid gap-3 rounded-lg border bg-gray-50 p-4 sm:grid-cols-2"><p><span className="text-gray-500">Employee:</span> {actorName(selected.actor)} ({selected.actor?.email || 'system'})</p><p><span className="text-gray-500">Action:</span> {selected.action}</p><p><span className="text-gray-500">Source:</span> {selected.source}</p><p><span className="text-gray-500">Record:</span> {selected.entityType} · {selected.entityId || 'multiple'}</p><p><span className="text-gray-500">Time:</span> {new Date(selected.createdAt).toLocaleString()}</p><p><span className="text-gray-500">IP:</span> {selected.ip || '—'}</p><p><span className="text-gray-500">Request ID:</span> <span className="font-mono text-xs">{selected.requestId || '—'}</span></p>{selected.message && <p className="sm:col-span-2"><span className="text-gray-500">Message:</span> {selected.message}</p>}</div>{selected.before != null && <JsonPanel title="Previous values" value={selected.before} />}<JsonPanel title="Fields submitted / changed" value={selected.diff} /><JsonPanel title="Saved result" value={selected.after} /></div>}</DialogContent></Dialog>
  </div>;
}
