...
export default function StockMovementsPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setTotal(movRes.data.data.total);
  ...
  const columns = [
    ...
    { key: 'product', header: 'Product', render: (m: StockMovement) => <span className="font-medium">{m.product?.name || '—'}</span> },
    { key: 'warehouse', header: 'Warehouse', render: (m: StockMovement) => m.warehouse?.name || '—' },
    { key: 'quantity', header: 'Qty', render: (m: StockMovement) => (
  ...
  return (
    <div>
      <PageHeader title="Stock Movements" description="Track inventory in and out" action={{ label: 'Record Movement', onClick: () => setShowModal(true), icon: Plus }} />
      <div className="flex gap-2 mb-4 flex-wrap">
        <Button variant={typeFilter === '' ? 'default' : 'outline'} size="sm" onClick={() => setTypeFilter('')}>All</Button>
        {movTypes.map(t => <Button key={t} variant={typeFilter === t ? 'default' : 'outline'} size="sm" onClick={() => setTypeFilter(t)}>{t}</Button>)}
      </div>
      {movements.length === 0 && !isLoading ? (
        <EmptyState icon={ArrowUpDown} title="No movements" description="Record your first stock movement" action={{ label: 'Record Movement', onClick: () => setShowModal(true) }} />
      ) : (
        <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select value={form.type} onValueChange={v => setForm(f => ({ ...f, type: v }))}>
                  ...
                  <SelectContent>{movTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5"><Label>Quantity *</Label><Input type="number" min="1" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} required /></div>
            </div>
            <div className="space-y-1.5"><Label>Reference</Label><Input value={form.reference} onChange={e => setForm(f => ({ ...f, reference: e.target.value }))} placeholder="PO-001, INV-001..." /></div>
