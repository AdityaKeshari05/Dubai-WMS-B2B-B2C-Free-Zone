...
export default function PaymentDetailPage() {
  ...
  if (isLoading || !payment) {
    return <div className="space-y-3">{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-12" />)}</div>;
  }

  return (
    <div className="space-y-4">
      <Button variant="ghost" size="sm" asChild>
        <Link href="/invoicing/payments"><ArrowLeft className="mr-2 h-4 w-4" />Back to payments</Link>
      </Button>
      <Card>
        <CardHeader><CardTitle>{payment.paymentNo}</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
          <Info label="Type" value={payment.type} />
          <Info label="Customer" value={payment.customer?.name || payment.customerId || '-'} />
          <Info label="Invoice" value={payment.invoice?.invoiceNo || payment.invoiceId || '-'} href={payment.invoiceId ? `/invoicing/sales-invoices/${payment.invoiceId}` : undefined} />
...
function Info({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div className="rounded-md border border-[#e5e2dc] bg-[#f8faf9] px-3 py-2">
      ...
      {href ? <Link href={href} className="mt-1 block text-sm font-medium text-[#1674c4] hover:underline">{value}</Link> : <p className="mt-1 text-sm font-medium text-[#1f2937]">{value}</p>}
    </div>
  );
}
