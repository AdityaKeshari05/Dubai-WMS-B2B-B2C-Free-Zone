...
export default function BackordersPage() {
  ...
  // Only show backordered orders
  const orders = useMemo(() => allOrders.filter(o => o.status === 'backordered'), [allOrders]);

  return (
    <div>
      <PageHeader 
        title="6.6 Backorders" 
        description="Orders pending later fulfillment due to unavailable quantities"
      />

      <DataTable
        data={orders}
        onRowClick={(o) => router.push(`/wms/b2b/${o.id}`)}
        emptyMessage="No backorders currently exist."
        columns={[
          ...
          { key: 'backordered', header: 'Pending Qty', render: (o) => {
