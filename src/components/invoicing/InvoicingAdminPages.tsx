...
import { FileText, Plus, RotateCcw, Trash2, Play, Printer, Repeat, Settings2 } from 'lucide-react';
        ...
        <DialogContent className="max-w-3xl">
          ...
          <div className="grid gap-3">
            <div className="grid gap-3 md:grid-cols-3">
              <Field label="Name"><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="GST 18% Intrastate" /></Field>
              <Field label="Code"><Input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="GST-18-INTRA" /></Field>
              <Field label="Scope">
                <Select value={form.scope} onValueChange={scope => setForm(f => ({ ...f, scope }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{['ITEM', 'INVOICE', 'BOTH'].map(scope => <SelectItem key={scope} value={scope}>{scope}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
            </div>
            <Field label="Description"><Textarea rows={2} value={form.description} onChange={event => setForm(prev => ({ ...prev, description: event.target.value }))} placeholder="Usage note, jurisdiction, GST treatment, export rule..." /></Field>
            <div className="rounded-md border border-[#e5e2dc] bg-white">
              <div className="grid grid-cols-[2fr_110px_190px_120px_44px] border-b border-[#e5e2dc] bg-[#f8faf9] px-3 py-2 text-xs font-semibold uppercase text-[#6b7280]">
                <span>Tax Row</span><span>Rate %</span><span>Charge Type</span><span>Recoverable</span><span />
              </div>
              <div className="divide-y divide-[#f0ede8]">
                {lines.map((line, index) => (
                  <div key={index} className="grid grid-cols-[2fr_110px_190px_120px_44px] gap-2 px-3 py-2">
                    <Input value={line.label} onChange={event => setLines(prev => prev.map((row, i) => i === index ? { ...row, label: event.target.value } : row))} placeholder="CGST" />
                    <Input type="number" step="0.01" value={line.rate} onChange={event => setLines(prev => prev.map((row, i) => i === index ? { ...row, rate: Number(event.target.value) } : row))} />
                    <Select value={line.chargeType} onValueChange={chargeType => setLines(prev => prev.map((row, i) => i === index ? { ...row, chargeType } : row))}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>{['ON_NET_TOTAL', 'ON_PREVIOUS_ROW_AMOUNT', 'ON_PREVIOUS_ROW_TOTAL'].map(type => <SelectItem key={type} value={type}>{type.replaceAll('_', ' ')}</SelectItem>)}</SelectContent>
                    </Select>
                    <Button type="button" variant={line.isRecoverable ? 'success' : 'outline'} onClick={() => setLines(prev => prev.map((row, i) => i === index ? { ...row, isRecoverable: !row.isRecoverable } : row))}>{line.isRecoverable ? 'Yes' : 'No'}</Button>
                    <Button type="button" variant="ghost" size="icon" onClick={() => setLines(prev => prev.length === 1 ? prev : prev.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                ))}
              </div>
              <div className="border-t border-[#e5e2dc] p-2"><Button type="button" size="sm" variant="outline" onClick={() => setLines(prev => [...prev, { label: '', rate: 0, chargeType: 'ON_NET_TOTAL', isRecoverable: false }])}><Plus className="mr-2 h-4 w-4" />Add Tax Row</Button></div>
            </div>
...
export function CreditNotesPage() {
  ...
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ originalInvoiceId: '', reason: 'SALES_RETURN', date: new Date().toISOString().slice(0, 10), notes: '' });
  const [rows, setRows] = useState<any[]>([{ productId: '', itemCode: '', description: '', quantity: 1, unitPrice: 0, discount: 0, taxRate: 0 }]);
  ...
  const selectInvoice = (invoiceId: string) => {
    ...
    if (invoice?.items?.length) {
      setRows(invoice.items.map((item: any) => ({
  ...
  const create = async () => {
    if (!form.originalInvoiceId) return toast.error('Select the original submitted invoice');
    const items = rows.filter(row => row.productId && Number(row.quantity) > 0).map(row => ({
    ...
    if (!items.length) return toast.error('Add at least one credit item');
    try {
  ...
  return (
    <CrudShell title="Credit Notes" description="Create customer credits from submitted invoices, then submit to reduce receivables and post ledger reversal." actionLabel="New Credit Note" onAction={() => setOpen(true)}>
      {notes.length ? (
      ...
      ) : <EmptyState icon={FileText} title="No credit notes" description="Create a credit note from a submitted invoice for returns, corrections, or goodwill credits." action={{ label: 'New Credit Note', onClick: () => setOpen(true) }} />}
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Field label="Original Invoice">
                <Select value={form.originalInvoiceId} onValueChange={selectInvoice}>
                  <SelectTrigger><SelectValue placeholder="Select submitted invoice" /></SelectTrigger>
                  <SelectContent>{invoices.map(invoice => <SelectItem key={invoice.id} value={invoice.id}>{invoice.invoiceNo} · {invoice.customer?.name || 'Customer'} · {formatCurrency(invoice.grandTotal || invoice.total, invoice.currency)}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Reason">
                <Select value={form.reason} onValueChange={reason => setForm(prev => ({ ...prev, reason }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{['SALES_RETURN', 'RATE_ADJUSTMENT', 'DISCOUNT', 'TAX_CORRECTION', 'FULL_REVERSAL', 'OTHER'].map(reason => <SelectItem key={reason} value={reason}>{reason.replaceAll('_', ' ')}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Date"><Input type="date" value={form.date} onChange={event => setForm(prev => ({ ...prev, date: event.target.value }))} /></Field>
              <div className="md:col-span-3"><Field label="Notes"><Textarea rows={2} value={form.notes} onChange={event => setForm(prev => ({ ...prev, notes: event.target.value }))} placeholder="Return reason, correction reference, approval note" /></Field></div>
            </div>

            {selectedInvoice && (
              <div className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-sm text-blue-800">
                Crediting against {selectedInvoice.invoiceNo}. Invoice total: {formatCurrency(selectedInvoice.grandTotal || selectedInvoice.total, selectedInvoice.currency)}.
              </div>
            )}

            <CreditRows rows={rows} onChange={setRows} currency={selectedInvoice?.currency || 'USD'} />
            <div className="flex items-center justify-between">
              <div className="rounded-md border border-[#e5e2dc] bg-[#f8faf9] px-3 py-2 text-sm">
                Draft credit preview: <span className="font-semibold">{formatCurrency(previewTotal, selectedInvoice?.currency || 'USD')}</span>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
                <Button onClick={create}><RotateCcw className="mr-2 h-4 w-4" />Create Draft Credit</Button>
              </div>
            </div>
          </div>
        </DialogContent>
  ...
  return (
    <div className="rounded-md border border-[#e5e2dc] bg-white">
      <div className="grid grid-cols-[1.2fr_2fr_90px_110px_90px_90px_120px_44px] border-b border-[#e5e2dc] bg-[#f8faf9] px-3 py-2 text-xs font-semibold uppercase text-[#6b7280]">
        <span>Item</span><span>Description</span><span>Qty</span><span>Rate</span><span>Disc %</span><span>Tax %</span><span>Total</span><span />
      </div>
      <div className="divide-y divide-[#f0ede8]">
        {rows.map((row, index) => {
          const net = Number(row.quantity || 0) * Number(row.unitPrice || 0) * (1 - Number(row.discount || 0) / 100);
          ...
          return (
            <div key={index} className="grid grid-cols-[1.2fr_2fr_90px_110px_90px_90px_120px_44px] gap-2 px-3 py-2">
              <Input value={row.itemCode || ''} onChange={event => update(index, { itemCode: event.target.value })} readOnly />
              ...
              <Input type="number" min="0" step="0.01" value={row.unitPrice} onChange={event => update(index, { unitPrice: Number(event.target.value) })} />
              <Input type="number" min="0" step="0.01" value={row.discount} onChange={event => update(index, { discount: Number(event.target.value) })} />
              <Input type="number" min="0" step="0.01" value={row.taxRate} onChange={event => update(index, { taxRate: Number(event.target.value) })} />
...
export function PrintFormatsPage() {
  ...
  const create = async () => {
    ...
    }).then(() => { setOpen(false); fetchFormats(); }).catch((err) => toast.error(err.response?.data?.message || 'Failed to create format'));
  };

  const patchFormat = async (id: string, payload: any) => {
    await api.patch(`/invoicing/print-formats/${id}`, payload)
      .then(fetchFormats)
      .catch((err) => toast.error(err.response?.data?.message || 'Failed to update format'));
  };

  return (
    <CrudShell title="Print Formats" description="Reusable PDF layouts. The default active Sales Invoice format is used by the invoice PDF endpoint." actionLabel="New Format" onAction={() => setOpen(true)}>
      <DataTable data={formats} columns={[
        { key: 'name', header: 'Name' },
        { key: 'docType', header: 'Document' },
        { key: 'template', header: 'Layout', render: (f: any) => f.template?.title || f.template?.sections?.join(', ') || 'Standard' },
        { key: 'isDefault', header: 'Default', render: (f: any) => f.isDefault ? <StatusBadge status="DEFAULT" /> : <Button size="sm" variant="outline" onClick={() => patchFormat(f.id, { isDefault: true })}>Make Default</Button> },
        { key: 'isActive', header: 'Status', render: (f: any) => <StatusBadge status={f.isActive ? 'ACTIVE' : 'INACTIVE'} /> },
        { key: 'actions', header: '', render: (f: any) => (
          <Button size="sm" variant="outline" onClick={() => patchFormat(f.id, { isActive: !f.isActive })}>{f.isActive ? 'Disable' : 'Enable'}</Button>
        ) },
      ]} />
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>New Print Format</DialogTitle></DialogHeader>
          <div className="grid gap-4">
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <Field label="Document">
                <Select value={form.docType} onValueChange={docType => setForm(f => ({ ...f, docType }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{['SALES_INVOICE', 'CREDIT_NOTE', 'PAYMENT_ENTRY'].map(type => <SelectItem key={type} value={type}>{type.replace('_', ' ')}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Title"><Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} /></Field>
            </div>
            <Field label="Footer"><Textarea rows={2} value={form.footer} onChange={e => setForm(f => ({ ...f, footer: e.target.value }))} /></Field>
            <div className="grid gap-2 md:grid-cols-4">
              <ToggleButton active={form.isDefault} label="Default" onClick={() => setForm(f => ({ ...f, isDefault: !f.isDefault }))} />
              <ToggleButton active={form.showLogo} label="Logo" onClick={() => setForm(f => ({ ...f, showLogo: !f.showLogo }))} />
              <ToggleButton active={form.showHsnSummary} label="HSN Summary" onClick={() => setForm(f => ({ ...f, showHsnSummary: !f.showHsnSummary }))} />
              <ToggleButton active={form.showAmountInWords} label="Amount in Words" onClick={() => setForm(f => ({ ...f, showAmountInWords: !f.showAmountInWords }))} />
            </div>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={create}><Printer className="mr-2 h-4 w-4" />Create Format</Button></div>
          </div>
        </DialogContent>
      </Dialog>
    </CrudShell>
...
export function SubscriptionsPage() {
  ...
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ name: '', customerId: '', frequency: 'MONTHLY', startDate: new Date().toISOString().slice(0, 10), nextRunDate: new Date().toISOString().slice(0, 10), endDate: '', currency: 'USD', autoSubmit: false, notes: '', terms: '' });
  ...
  useEffect(() => {
    fetchSubscriptions();
    api.get('/customers', { params: { limit: 200, isActive: true } }).then(res => setCustomers(asItems(res.data))).catch(() => toast.error('Failed to load customers'));
  }, []);
  ...
  const createSubscription = async () => {
    ...
    if (!items.length) return toast.error('Add at least one subscription item');
    await api.post('/invoicing/subscriptions', { ...form, endDate: form.endDate || undefined, items })
      .then(() => {
        toast.success('Subscription created');
        setOpen(false);
        fetchSubscriptions();
      })
      .catch((err) => toast.error(err.response?.data?.message || 'Failed to create subscription'));
  };
  ...
  const toggleSubscription = async (subscription: any) => {
    await api.patch(`/invoicing/subscriptions/${subscription.id}`, { isActive: !subscription.isActive })
      ...
      .catch((err) => toast.error(err.response?.data?.message || 'Failed to update subscription'));
  };

  return (
    <CrudShell title="Recurring Invoices" description="Templates that generate real sales invoices on schedule, with optional auto-submit." actionLabel="New Subscription" onAction={() => setOpen(true)} extraAction={<RecurringRunnerButton onDone={fetchSubscriptions} />}>
      {subscriptions.length ? <DataTable data={subscriptions} columns={[
        ...
        { key: 'generatedInvoices', header: 'Generated', render: (s: any) => s.generatedInvoices?.length ? s.generatedInvoices.map((run: any) => run.invoice?.invoiceNo).filter(Boolean).join(', ') : '-' },
        { key: 'isActive', header: 'Status', render: (s: any) => <StatusBadge status={s.isActive ? 'ACTIVE' : 'INACTIVE'} /> },
        { key: 'actions', header: '', render: (s: any) => <Button size="sm" variant="outline" onClick={() => toggleSubscription(s)}>{s.isActive ? 'Pause' : 'Resume'}</Button> },
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-5xl">
          ...
          <div className="grid gap-4">
            <div className="grid gap-3 md:grid-cols-3">
              <Field label="Name"><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Monthly support retainer" /></Field>
              <Field label="Customer">
                <Select value={form.customerId} onValueChange={customerId => {
                  const customer = customers.find(c => c.id === customerId);
                  setForm(f => ({ ...f, customerId, currency: customer?.currency || f.currency }));
                }}>
              ...
              <Field label="Frequency">
                <Select value={form.frequency} onValueChange={frequency => setForm(f => ({ ...f, frequency }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{['MONTHLY', 'QUARTERLY', 'ANNUALLY'].map(frequency => <SelectItem key={frequency} value={frequency}>{frequency}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Start Date"><Input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value, nextRunDate: f.nextRunDate || e.target.value }))} /></Field>
              <Field label="Next Run"><Input type="date" value={form.nextRunDate} onChange={e => setForm(f => ({ ...f, nextRunDate: e.target.value }))} /></Field>
              ...
              <ToggleButton active={form.autoSubmit} label="Auto submit invoices" onClick={() => setForm(f => ({ ...f, autoSubmit: !f.autoSubmit }))} />
            </div>
            <LineItemGrid value={rows} onChange={setRows} currency={form.currency} />
            <div className="flex items-center justify-between">
              <div className="rounded-md border border-[#e5e2dc] bg-[#f8faf9] px-3 py-2 text-sm">Recurring invoice preview: <span className="font-semibold">{formatCurrency(summary.total, form.currency)}</span></div>
              <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={createSubscription}><Repeat className="mr-2 h-4 w-4" />Create Subscription</Button></div>
            </div>
          </div>
        </DialogContent>
      </Dialog>
...
function ToggleButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return <Button type="button" variant={active ? 'success' : 'outline'} onClick={onClick}><Settings2 className="mr-2 h-4 w-4" />{label}</Button>;
}
