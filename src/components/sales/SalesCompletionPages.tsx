...
export function SalesEnquiriesPage() {
  ...
  const create = async () => { try { const payload = { customerId: form.customerId, requirements: form.requirements, targetDate: form.targetDate || undefined, items: [{ productId: form.productId, targetQty: Number(form.targetQty), targetPrice: form.targetPrice ? Number(form.targetPrice) : undefined }] }; if (editingId) await api.put(`/sales/enquiries/${editingId}`, payload); else await api.post('/sales/enquiries', payload); showApiSuccess(`Sales enquiry ${editingId ? 'updated' : 'created'}`); setEditingId(null); setForm({ customerId: '', productId: '', requirements: '', targetQty: '1', targetPrice: '', targetDate: '' }); load(); } catch (e) { showApiError(e, 'Could not save enquiry'); } };
  const edit = (row: any) => { const item = row.items?.[0] || {}; setEditingId(row.id); setForm({ customerId: row.customerId, productId: item.productId || '', requirements: row.requirements || '', targetQty: String(item.targetQty || 1), targetPrice: item.targetPrice == null ? '' : String(item.targetPrice), targetDate: row.targetDate ? String(row.targetDate).slice(0, 10) : '' }); };
  ...
  return <div><PageHeader title="Sales Enquiries" description="Customer requirements and target quantity, date and price before quotation" />
    <Card className="mb-5"><CardHeader><CardTitle>{editingId ? 'Edit' : 'New'} enquiry</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-3">
